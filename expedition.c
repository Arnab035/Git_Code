...
int main(){
    ...
    while(testcases-- > 0){
                          scanf("%d",&numbers);
                          struct station stops[numbers];    //stops is an array of type struct station, it is an array of structures
                          for(i=0;i<numbers;i++){
...
void partition(struct station array[],int low,int high){
     int mid;
     if(low < high){
            mid = (low+high)/2;
